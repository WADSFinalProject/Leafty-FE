...
function TableComponent({ data, header, columns, ColorConfig, admin = false, rows = 10, depends='status', onDelete}) {
  ...
  const actionTemplate = (rowData, admin) => {
    return (
      <>
        <div className="dropdown dropdown-end">
          ...
          <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
            <li><a>Details</a></li>
            {admin ? <><li><a>Edit</a></li><li><a onClick={() => onDelete(rowData.id)}>Remove</a></li></> : <></>}
          </ul>
        </div>
      </>
    );
  ...
  return (
    <div className="container w-full">
      ...
      >
        ...
        <Column key="action" header="Action" body={(rowData) => actionTemplate(rowData, admin)} />
      </DataTable>
    </div>
  );
}
