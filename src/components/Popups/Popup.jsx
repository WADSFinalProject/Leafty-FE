...
const Popup = forwardRef(({ warning = false, info = false, error = false, confirm = false, leavesid, description }, ref) => {
    return (
        <dialog ref={ref} id={leavesid} className="modal">
            <div className="modal-box rounded-lg flex items-start flex-col gap-2 items-center">
                ...
                <Button className="w-full flex-grow p-0" type="submit" background="#0F7275" color="#F7FAFC" label="Confirm" />
                {confirm && (
                    <div className='gap-2 w-full flex flex-col items-center'>     
                        <Button className="w-full flex-grow" type="cancel" background="#94C3B3" color="#F7FAFC" label="Cancel" />
                    </div>
                )}
